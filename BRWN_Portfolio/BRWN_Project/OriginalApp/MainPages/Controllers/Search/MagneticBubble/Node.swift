...
//            let image = UIImage(data: try! Data(contentsOf: url))
    ...
    */
//Nodeタップ後の大きさを調節~~~~~~~
//    open var selectedScale: CGFloat = CGRect(
    ...
    */
    ...
    struct Defaults {
        ...
        static let marginScale = CGFloat(1.0) //丸の透明の枠~~~~~~
...
//サイズ調整~~~~~~
    public func resize() {
        ...
        let radius = size.width / 1 + CGFloat(padding)
...
//        let scaleAction = SKAction.scale(to: selectedScale, duration: animationDuration)
        let screenSizeX = UIScreen.main.bounds.maxX / CGFloat(animationDuration)
        let screenSizeY = UIScreen.main.bounds.maxY / CGFloat(animationDuration)
        
        let view = UIView()

//④~~~~~~~~~~
//        let moveAction = SKAction.move(to: CGPoint(x:1000, y:1000), duration:1.0)
        let scaleAction = SKAction.scaleX(to: 50, y: 100, duration: 3.0)
        scaleAction.timingMode = .easeIn
//        let rectangle = SKShapeNode(rect: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.maxX, height: UIScreen.main.bounds.maxY))
...
//            run(moveAction)
    ...
    open func deselectedAnimation() {
        let scaleAction = SKAction.scale(to: deselectedScale, duration: 2.0)
